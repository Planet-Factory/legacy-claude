...
from scipy.interpolate.fitpack2 import SmoothBivariateSpline, SmoothSphereBivariateSpline
...
class Pixel:
    ...
    def update_temp(self, dt: int, solar_constant: int, sun_lon: float) -> None:
        self.temp += dt * (
    ...
    def update_velocity(self,
                        dt: int,
                        temp: SmoothSphereBivariateSpline,
                        u: SmoothSphereBivariateSpline,
                        v: SmoothSphereBivariateSpline) -> None:
...
def plotting():
    ...
    print(f'T: {atmosp_temp.max() - 273.15:.1f} - {atmosp_temp.min() - 273.15:.1f} C')
    print(f'U: {U.max():.2f} - {U.min():.2f} V: {V.max():.2f} {V.min():.2f}')
...
while True:
    ...
    print(f'TIME: {str(time / day).zfill(2)} days')
